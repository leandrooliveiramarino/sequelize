...
const QueryGenerator = {
  ...
  */
  insertQuery(table, valueHash, modelAttributes, options) {
    ...
    options.onDuplicate = options.updateOnDuplicate
      ? options.updateOnDuplicate
      : false;

    if (this._dialect.supports['ON DUPLICATE KEY'] && options.onDuplicate) {
      const self = this;
      let duplicatedValuesTreated = options.onDuplicate.filter(function(field) {
        return valueHash[field] !== null && valueHash[field] !== undefined && valueHash[field] !== ''; //Prevent adding nullable fields
      }).map(function(field) {
        const value = (valueHash[field] !== undefined) //Accepting false (boolean) value
          ? valueHash[field]
          : null;
        return `${field} = ${self.escape(value)}`;
      });

      if(options.onDuplicate.indexOf('id') > -1) {
        if(valueHash['id']) {
          duplicatedValuesTreated.push('id = ' + valueHash['id']);
        }
      } else {
          duplicatedValuesTreated.push('id = LAST_INSERT_ID(id)');
      }

      const duplicatedValues = duplicatedValuesTreated.join(',');

      valueQuery += ' ON DUPLICATE KEY UPDATE ' + duplicatedValues;
      emptyQuery += ' ON DUPLICATE KEY UPDATE ' + duplicatedValues;
    }
